...
export const ProgramCard = () => {
  ...
  useEffect(() => {
    setLoading(true)
    axios
      .get("http://13.232.130.207/learn/program/all")
      .then((res) => {
        setCard(res.data)
        setLoading(false)
  ...
  return (
    <Flex gap={10} className="offerCardContainer" marginY={10}>
      { loading ? <Spinner  marginLeft={"650px"} marginTop={20} thickness='4px' speed='0.65s' emptyColor='gray.200' color='red.500' size='xl'/> :  card?.map((e) => {
        return (
          <Box px={1} key={e.id} className="offerCard">
            <Box
              className="offerCardOne"
              w={"432px"}
              bg={"white"}
              boxShadow={"xl"}
              rounded={"md"}
