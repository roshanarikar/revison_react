...
export const Page2 = () => {
 ...
 const removeItem = () =>{
  ...
  console.log("seachResult", searchQuery)
  return (
    <>
    <Navbar/>
          ...
          <Box key={item.id}  border={"1px solid grey"} w={400} h={300} borderRadius={20} _hover={{boxShadow:"rgba(0, 0, 0, 0.35) 0px 5px 15px"}}>
            ...
            <Flex>
               <Box bgColor={"rgb(16,53,124)"} w={400} h={190} borderTopRadius={20} paddingLeft={"50px"} paddingTop={"55px"}>
                    <Text paddingLeft={"75px"} fontSize={20} color={"grey"} fontWeight={'bold'}>{item.moduleName}</Text>
                    <Text paddingLeft={"10px"} fontSize={25} color={'white'} fontWeight={'bold'}>{item.tutorialOf}</Text>
                </Box>               
            </Flex>
            <Flex flexDirection={'column'} margin={'auto'} w={"95%"} paddingY={5}>
                ...
                <span style={{ width:"200px",backgroundColor:'#F2FDFF', color:'#1A9FBD', borderRadius:"10px", paddingLeft:"10px"}}>{item.tutorialOf}</span>
                <Text fontSize={22} fontWeight={'bold'}>{item.heading}</Text>
            </Flex>
            </NavLink>
          </Box>
        ))
