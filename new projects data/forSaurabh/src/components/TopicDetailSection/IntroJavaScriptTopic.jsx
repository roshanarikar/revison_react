import { Accordion, AccordionButton, AccordionIcon, AccordionItem, AccordionPanel, Box, Button, Flex, Grid, GridItem, Heading, Image, Stack, Text } from "@chakra-ui/react"
...
export const IntroJavaScriptTopic = () =>{
    const [Topic, setTopic] = useState([]);
    const [modules, setModules] = useState([]);

    useEffect(() => {
      axios
        .get("http://localhost:8080/introJavascript")
  ...
  useEffect(() => {
    ...
    return(
        ...
        <Stack marginTop={20} >
            <Box marginX={10}>
              {/* <img src='https://masai-website-images.s3.ap-south-1.amazonaws.com/Introduction_to_Java_Script_Desktop_da858df529.jpg' alt="Topic Details" /> */}
             <img src={Topic?.imageDesktop?.url} alt="Topic Details" />
            </Box>
            <Box>
            <Flex
                as="nav"
        ...
        <Box color={"red"}>
          <NavLink to={"/tutorials/javascript-tutorial/introduction-to-javascript"} _hover={{ textDecoration: "none" }}>
            JavaScript Tutorial
          </NavLink>
        </Box> 
        </Flex>
            ...
            <Flex margin={"auto"}>
                ...
                <Box w={1000} marginX={10} paddingX={5} marginY={10}>
                    <Box>
                        <Heading size={"2xl"} fontWeight={"bold"}>{Topic.heading}</Heading>
                        <Flex fontSize={18} flexDirection={"row"} color={"grey"} gap={5} marginY={5}>
                            <Text>By {Topic.author}</Text>
                            <Text> | </Text>
                            <Text>Updated on : {Topic.updatedOn}</Text>
                            <Text> | </Text>
                            <Text>{Topic.readingTime}</Text>
                            <Text> | </Text>
                            <Text>Published on : {Topic.publishedOn}</Text>
                        </Flex>
                    </Box>
                    <Box borderBottomWidth={1} borderColor={"grey"}>
                    {
                        Topic?.subTopics?.map((e)=>{
                            return(
                                <Box  key={e.id} marginY={5}>
                                 <Heading size={"xl"}>{e.subTopicHeading}</Heading>
                                 <Text fontSize={18} paddingY={3} lineHeight={7} whiteSpace="pre-wrap">{e.description}</Text>
                                </Box>
                            )
                        })
                    }
