...
export const TopicWhyJavaScript = () =>{
  ...
  useEffect(() => {
    axios
      .get("http://13.232.130.207/learn/module/all?pageSize=4&pageNumber=1")
      .then((res) => {
    ...
    useEffect(() => {
      setLoading(true)
      axios
        .get("http://13.232.130.207/learn/topic/all?pageSize=1&pageNumber=2")
        .then((res) => {
    ...
    return(
        ...
        <Box display={"flex"} gap="2">
          <NavLink to={"/tutorials/javascript-tutorial"} _hover={{ textDecoration: "underline" }}>
            JavaScript Tutorial
          </NavLink>
          <Text>{">"}</Text>
        </Box> 
        <Box color={"red"}>
          <NavLink to={"/tutorials/javascript-tutorial/why-learn-javascript"} _hover={{ textDecoration: "none" }}>
          Introduction
          </NavLink>
        </Box> 
        </Flex>
            ...
            <Flex margin={"auto"}>
                ...
                <Box key={e.id} w={1000} marginX={10} paddingX={5} marginY={10}>
                    ...
                    { e.subTopics.map((item)=>{
                      ...
                      <Heading size={"xl"}>{ item.subTopicHeading ? item.subTopicHeading==null : item.subTopicHeading  }</Heading>
